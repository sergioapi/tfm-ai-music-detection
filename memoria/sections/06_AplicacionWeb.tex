...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{fig/verison_inicio.png}
    \caption{Pantalla inicial de VeriSon tras la selección de un archivo de audio.}
    \label{fig:verison-inicio}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{fig/version_human.png}
    \caption{Resultado de VeriSon con la etiqueta \textit{Posible origen humano}.}
    \label{fig:verison-humano}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{fig/version_ia.png}
    \caption{Resultado de VeriSon con la etiqueta \textit{Posible generación con IA}.}
    \label{fig:verison-ia}
\end{figure}
\endgroup
...
El workflow de GitHub Actions se ejecuta ante cada \textit{pull request} a \textit{main} y cada \textit{push} a \textit{main}. Incluye los siguientes \textit{checks}:

\begin{itemize}
    \item \textbf{Backend}: utiliza Python 3.11.9, instala las dependencias y ejecuta la suite de tests del backend.
    \item \textbf{Frontend}: utiliza Node 20.19.0, instala las dependencias y ejecuta las comprobaciones del frontend, incluidos el análisis estático, el \textit{build} y los tests.
...
La protección de \textit{main} requiere una \textit{pull request}, y los \textit{checks} obligatorios deben finalizar correctamente antes del \textit{merge}. GitHub Actions se utiliza exclusivamente como mecanismo de integración continua, el workflow no publica la imagen Docker ni realiza los despliegues en Northflank o Vercel. Su función es verificar el código, las pruebas y el empaquetado antes de integrar cambios en \textit{main}, por lo que no constituye un pipeline completo de CI/CD.
