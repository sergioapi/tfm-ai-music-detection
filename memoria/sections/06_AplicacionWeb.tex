...
\begin{itemize}
    \item \textbf{Backend}: utiliza Python 3.11.9, instala las dependencias y ejecuta la suite implementada con \textit{pytest}.

    \item \textbf{Frontend}: utiliza Node 20.19.0, instala las dependencias y ejecuta \textit{npm run check}, un comando del proyecto que agrupa el \textit{lint}, la comprobación del \textit{build} y las pruebas automatizadas con \textit{Vitest}.
...
\section{Pruebas y validación de la aplicación}

La validación combinó pruebas automatizadas de backend y frontend, la construcción de la imagen Docker y pruebas manuales sobre la aplicación desplegada. En conjunto, se superaron 191 tests automatizados. La Tabla~\ref{tab:pruebas-validacion-aplicacion} resume los ámbitos cubiertos y los resultados obtenidos.

\begin{table}[H]
    \centering
    \caption{Resumen de las pruebas y validación de VeriSon.}
    \label{tab:pruebas-validacion-aplicacion}
    \begin{tabularx}{\textwidth}{
        ...
        \toprule
        \textbf{Ámbito} & \textbf{Tipo de prueba} & \textbf{Qué verifica} & \textbf{Resultado} \\
        \midrule
        Backend & Automatizada & Procesamiento de audio, inferencia, endpoints y validaciones de la API. & 162 tests superados. \\
        Frontend & Automatizada & Selección de archivos, estados de la interfaz, comunicación con la API y presentación de resultados. & 29 tests superados. \\
        Docker & Automatizada & Construcción de la imagen de despliegue del backend. & Build superado. \\
        Aplicación desplegada & Manual & Flujos válidos e inválidos, límites, indisponibilidad del backend y diseño responsive. & Casos comprobados satisfactoriamente. \\
        \bottomrule
    \end{tabularx}
\end{table}
